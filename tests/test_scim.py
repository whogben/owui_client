...
async def test_get_scim_user(client):
    ...
    created = await client.scim.create_user(user_data)
    assert created is not None

    try:
        fetched = await client.scim.get_user(created.id)
...
async def test_create_scim_user(client):
    ...
    assert created.displayName == user_data.displayName
...
async def test_update_scim_user(client):
    ...
    try:
        ...
        updated = await client.scim.update_user(created.id, update_data)
...
async def test_patch_scim_user(client):
    ...
    try:
        ...
        patched = await client.scim.patch_user(created.id, patch_data)
...
async def test_delete_scim_user(client):
    ...
    deleted = await client.scim.delete_user(created.id)
    assert deleted is True

    try:
        await client.scim.get_user(created.id)
...
async def test_get_scim_group(client):
    ...
    created = await client.scim.create_group(group_data)
    assert created is not None

    try:
        fetched = await client.scim.get_group(created.id)
        assert fetched is not None
        assert fetched.id == created.id
...
async def test_update_scim_group(client):
    ...
    try:
        ...
        updated = await client.scim.update_group(created.id, update_data)
        assert updated is not None
...
async def test_patch_scim_group(client):
    ...
    try:
        ...
        patched = await client.scim.patch_group(created.id, patch_data)
        assert patched is not None
...
async def test_delete_scim_group(client):
    ...
    deleted = await client.scim.delete_group(created.id)
    assert deleted is True

    try:
        await client.scim.get_group(created.id)
